...
__global__ void morphology_3x3_kernel(
    ...
    unsigned char result = dilate != 0 ? 0 : 255;
    for (int kernel_y = -1; kernel_y <= 1; ++kernel_y) {
        for (int kernel_x = -1; kernel_x <= 1; ++kernel_x) {
            ...
            const unsigned char value = input[source_y * width + source_x];
            if (dilate != 0) {
...
int run_morphology(
    ...
    if (error == cudaSuccess) {
        ...
        morphology_3x3_kernel<<<blocks, threads>>>(
            device_input, device_output, width, height, dilate
        );
...
int cuda_dilate_3x3_u8(
    const unsigned char* input,
    unsigned char* output,
    int width,
    int height
) {
    return run_morphology(input, output, width, height, 1);
}

int cuda_erode_3x3_u8(
    const unsigned char* input,
    unsigned char* output,
    int width,
    int height
) {
    return run_morphology(input, output, width, height, 0);
}
