...
class MatchTiming(ctypes.Structure):
    _fields_ = [
        ...
        ("evaluated_positions", ctypes.c_longlong),
...
class CudaPatternMatcher:
    def __init__(self, dll_path: Path) -> None:
        ...
        self.dll.cuda_pattern_match_zncc_u8.argtypes = [
        ...
        self.dll.cuda_pattern_match_zncc_u8.restype = ctypes.c_int
    ...
    def match(
        ...
        code = self.dll.cuda_pattern_match_zncc_u8(
        ...
        timing_data = {
            "DLL：Template 預處理": timing.host_prepare_ms,
            "GPU：Host → Device": timing.host_to_device_ms,
            "GPU：ZNCC kernel": timing.kernel_ms,
            "GPU：kernel launch 次數": int(timing.kernel_launch_count),
            "GPU：Device → Host": timing.device_to_host_ms,
            "CPU：排序與 NMS": timing.sort_nms_ms,
            "DLL：總耗時": timing.total_ms,
            "搜尋位置數": int(timing.evaluated_positions),
            "門檻以上候選數": int(timing.raw_candidate_count),
            "實際保存候選數": int(timing.stored_candidate_count),
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self.search_step_spin.setRange(1, 64)
        self.search_step_spin.setValue(1)
        ...
        self.max_candidates_spin.setValue(200000)
    ...
    def _build_ui(self) -> None:
        ...
        parameter_layout.addRow("分數門檻", self.threshold_spin)
        parameter_layout.addRow("搜尋步距 px", self.search_step_spin)
    ...
    def _validate_parameters(self) -> MatchParameters:
        ...
        return MatchParameters(
            ...
            search_step=int(self.search_step_spin.value()),
